...
export class ApiService {
  ...
  getData(): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/zy`);
  }
  getHierarchy(): Observable<TreeNode[]> {
    return this.http.get<TreeNode[]>(`${this.apiUrl}/tree2`);
  }
  ...
  getAllLevels(parentId: number[]): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/children/${parentId}`);
  }
  private dataSource = new BehaviorSubject<TreeNode[]>([]);
  data$ = this.dataSource.asObservable();

  setData(data: TreeNode[]) {
    this.dataSource.next(data);
  }
  getChildrenAndnbOrdre(parentId: string,nb:number): Observable<any> {
    return this.http.get<any>(`${this.apiUrl}/children/${parentId}/${nb}`);
  }
  getEmployeesByUnit(unitCode: string): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/employees?unitCode=${unitCode}`);
  }
